...
import type { DashboardInstance, DashboardSpec, SelectionChangeListener } from 'graphein';
...
export function DashboardCanvas({ spec, className, style, onReady, onSelectionChange }: DashboardCanvasProps) {
  const { theme, sketch } = useTheme();
  const themed = useMemo(() => applyChartTheme(spec, theme, sketch), [spec, theme, sketch]);
  const ref = useDashboard<HTMLDivElement>(themed, { onReady, onSelectionChange });
